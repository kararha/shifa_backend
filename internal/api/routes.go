...
func registerReviewRoutes(router *mux.Router, handler *handlers.ReviewHandler) {
    ...
    reviewRouter.HandleFunc("/doctor/{doctorId}", func(w http.ResponseWriter, r *http.Request) {
        vars := mux.Vars(r)
        r.URL.Query().Set("doctorId", vars["doctorId"])
        handler.ListReviews(w, r)
    }).Methods("GET")
    // Get reviews for a specific home care provider
    reviewRouter.HandleFunc("/home-care-provider/{providerId}", func(w http.ResponseWriter, r *http.Request) {
        vars := mux.Vars(r)
        r.URL.Query().Set("homeCareProviderId", vars["providerId"])
        handler.ListReviews(w, r)
    }).Methods("GET")
}
