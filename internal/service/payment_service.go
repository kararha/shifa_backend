...
func (s *paymentService) CreatePayment(ctx context.Context, payment *repository.Payment) error {
	...
	// Validate that at least one of consultation_id or home_care_visit_id is provided
	if payment.ConsultationID <= 0 && payment.HomeCareVisitID <= 0 {
		return errors.New("either consultation_id or home_care_visit_id must be provided")
	}
